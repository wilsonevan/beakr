...
import { ChartContainer, Split, HeaderSummary, SummaryContainer, chartColors } from './GradeBookStyles';
...
const TrendsTable = ({grades, courses}) => {
  ...
  const calcMonths = () => {
    ...
    if (dateFns.isBefore(firstMonth, lastMonth)){
      ...
      for ( currentMonth = firstMonth; dateFns.isBefore( currentMonth, dateFns.addMonths(lastMonth,1) ); currentMonth = dateFns.addMonths(currentMonth, 1) ) {
  ...
  const calcTotalGradesByMonth = (months, courseId) => {
    return months.map( month => {
      ...
      grades.map( grade => {
        if (grade.course_id === courseId && dateFns.isBefore(grade.due_date, dateFns.addMonths(month, 1))) {
  ...
  const generateChartData = () => {
    ...
    const displayMonths = months.map( month => {
      return dateFns.format(month, 'MM')
    })
  ...
  if (grades)
    return (
      <SummaryContainer>
        ...
        <Split />
        <ChartContainer>
          <Line data={generateChartData()} options={options} height={300} width={700} />
        </ChartContainer>
      </SummaryContainer>
    );
  else
