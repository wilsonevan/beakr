...
class SubmissionView extends React.Component {
  ...
  componentDidMount() {
    ...
    axios.get(`/api/assignments/${assignment_id}`).then(res => {
      this.setState({ assignment: res.data });
    });
    axios
      .get(`/api/assignments/${assignment_id}/assignment_submissions/${id}`)
      .then(res => {
        const {
          body,
          url,
    ...
    axios
      .get(
        `/api/assignments/${assignment_id}/assignment_submissions/${id}/find_user`
      )
      .then(res => {
  ...
  gradeSubmission = assignment_submission => {
    ...
    axios
      .put(
        `/api/assignments/${assignment_id}/assignment_submissions/${id}`,
        assignment_submission
      )
      .then(res => {
        this.setState({
          points_awarded: res.data.points_awarded,
        ...
        return (
          <Submission>
            <SubmissionControls>
              Submission
              <RightContainer>
                <Points>{points_awarded}</Points>
                <PointsPossible>/{points_possible}</PointsPossible>
              </RightContainer>
            </SubmissionControls>
            <SubmissionBody>
              Submission URL:{" "}
    ...
    return (
      <>
        ...
        <SubmissionContainer>
          <GradeHeading>
            <h2 style={{ margin: "0" }}>
              {graded ? (
                ...
                <span style={{ color: "#2979ff", margin: "0" }}>
                  {" "}
                  Not Graded{" "}
                </span>
              )}
          ...
          <StyledHr />
          <Instructions
            dangerouslySetInnerHTML={this.createMarkup(assignment.body)}
            style={{ padding: "15px" }}
          />
          <Divider />
